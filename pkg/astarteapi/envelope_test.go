...
func TestGoldenEnvelopes(t *testing.T) {
	cases := []struct {
		...
		{
			name: "data with links", golden: "data_links.json", wantStatus: 200,
			write: func(w http.ResponseWriter) error {
				return astarteapi.WriteDataWithLinks(w, 200, []string{"dT6hS2W9TT6LEnP25ks_lg"},
					astarteapi.Links{
		...
		{
			name: "data with links last page", golden: "data_links_last.json", wantStatus: 200,
			write: func(w http.ResponseWriter) error {
				return astarteapi.WriteDataWithLinks(w, 200, []string{"dT6hS2W9TT6LEnP25ks_lg"},
