...
verdict: proposed
at:  ff7905b
ran: 2026-09-18T11:41:01Z on DietPi in 222s
...
→ Read .mule/todo.md
✱ Grep "1\.2\.2|1\.3\.|1\.4\.|targets? up?stream|upstream (v)?version" in docs · 15 matches

$ git log --oneline -20
ff7905b mule: refill the queue
f3dd191 mule: recipe github-issues ran (206s)
...
$ ls .mule/ 2>/dev/null; echo "---reports---"; ls .mule/reports/ 2>/dev/null
MULE.md
config
cron.log
failed
...
$ gh api repos/astarte-platform/astarte/releases --jq '.[] | select(.tag_name=="v1.3.4") | {tag_name, published_at, body}'
{"body":"### Fixed\n\n- [astarte_data_updater_plant] Prevent AMQPDataConsumer processes from accumulating\n  uncollected binaries over time by forcing more frequent full sweep garbage collections\n  on them.\n- [astarte_data_updater_plant] Ensure the RPC server is always available to clients.\n","published_at":"2026-09-18T09:42:57Z","tag_name":"v1.3.4"}

→ Read .mule/for-giulio.md

$ git show 66f4932 --stat
commit 66f4932ecd0e64f6280cad0420a1e6bc7888264c
Author: Giulio Malitesta <atsetilam@gmail.com>
Date:   Thu Sep 17 21:07:03 2026 +0200

    mule: recipe astarte-upstream ran (106s)

 ...0260917T190650Z-recipe-astarte-upstream-noop.md |  44 +++
 ...917T190703Z-recipe-astarte-upstream-proposed.md | 294 +++++++++++++++++++++
 2 files changed, 338 insertions(+)

$ git show 66f4932 --name-only; echo "---"; git log --oneline --all -- .mule/reports/ | head -20
commit 66f4932ecd0e64f6280cad0420a1e6bc7888264c
Author: Giulio Malitesta <atsetilam@gmail.com>
Date:   Thu Sep 17 21:07:03 2026 +0200
...
$ git show 66f4932:422 | head -60 2>/dev/null; ls .mule/reports/
hygiene.md
log
fatal: path '422' does not exist in '66f4932'

$ git show 66f4932:.mule/reports/log/20260917T190703Z-recipe-astarte-upstream-proposed.md
slug: recipe-astarte-upstream
verdict: proposed
at:  5a314c7
...
Now I have the full picture. Let me check the exact end of the todo.md file and the tail of the reports log.
$ date -u +%Y%m%dT%H%M%SZ; git rev-parse --short HEAD
20260918T113930Z
ff7905b

→ Read .mule/todo.md [offset=160]
Context is clear. v1.3.4 shipped today (2026-09-18) — a new stable tag ahead of the 1.2.2 target. Let me assess the release notes against Astrate's surfaces and check what it changed upstream.

$ git show 5a314c7:.mule/reports/log/20260916T184218Z-recipe-astarte-upstream-proposed.md | sed -n '1,8p'
slug: recipe-astarte-upstream
verdict: proposed
at:  ed715ec
ran: 2026-09-16T18:42:18Z on DietPi in 121s
...
$ gh api "repos/astarte-platform/astarte/releases/tags/v1.3.4" --jq '.body' | head -20; echo '---compare v1.3.3..v1.3.4---'; gh api "repos/astarte-platform/astarte/compare/v1.3.3...v1.3.4" --jq '{total: .total_commits, commits: [.commits[].commit.message] | .[0:10]}'
### Fixed
...
{"commits":["fix(dup): prevent AMQP consumer process to accumulate uncollected bin… (#2194)\n\nSigned-off-by: Riccardo Nalgi \u003criccardo.nalgi@secomind.com\u003e\nCo-authored-by: Francesco Noacco \u003cfrancesco.noacco@secomind.com\u003e","chore!(dup): remove fullsweep_after configuration (#2198)\n\nSigned-off-by: Riccardo Nalgi \u003criccardo.nalgi@secomind.com\u003e","fix(dup): ensure RPC server is always available to clients (#2203)\n\nResolved the issue where a temporary disconnection and reconnection of data_updater_plant to the cluster would make the RPC server inaccessible.\n\nSigned-off-by: Francesco Noacco \u003cfrancesco.noacco@secomind.com\u003e","chore: prepare v1.3.4 (#2208)\n\nSigned-off-by: Riccardo Nalgi \u003criccardo.nalgi@secomind.com\u003e"],"total":4}

← Write .mule/reports/log/20260918T113930Z-recipe-astarte-upstream-proposed.md
Wrote file successfully.

Confirmed: v1.3.4 (today) is a maintenance patch — 3 changes, all in `astarte_data_updater_plant` (BEAM fullsweep GC tuning + RPC availability). Nothing wire-visible, no interface-schema change; both are Elixir-shaped internals Astrate doesn't map onto. So no substantive proposals qualify; only the recipe-mandated compat-note line.

Writing the evidence file and appending the one task line.
← Edit .mule/todo.md
...
$ git status --short; echo '---'; tail -2 .mule/todo.md
M .mule/todo.md
...
$ grep -n "compat-note-v1.3.4" .mule/todo.md
173:- [ ] compat-note-v1.3.4 [auto]: propose the docs/COMPATIBILITY.md wording for v1.3.4 (newest stable, 2026-09-18, maintenance-only patch — DUP fullsweep-GC tuning + RPC-availability fixes, no wire/API surface change; v1.4.0 is still rc.5-only) in .mule/for-giulio.md (do not edit the file) — the open v1.3.2 wording proposal in for-giulio.md stands; fold v1.3.3 and v1.3.4 in rather than re-deriving it.

Two files changed, zero git operations. Summary:

**v1.3.4** (2026-09-18, today) is the new upstream stable — a maintenance patch over v1.3.3, ahead of Astrate's 1.2.2 target. All 4 commits sit in `astarte_data_updater_plant`: BEAM fullsweep-GC tuning for AMQP consumer binary accumulation and RPC server availability after cluster reconnect. None is wire-visible, none survives restatement in Go. No substantive proposals qualify.

**Proposed (1 line):** `compat-note-v1.3.4` — the recipe mandates this as the last line on every upstream-ahead run, folding v1.3.3 + v1.3.4 into the open v1.3.2 wording proposal in `.mule/for-giulio.md`. It supersedes the existing blocked `compat-note-v1.3.3` line.

| Output | Path |
|---|---|
| Evidence report | `.mule/reports/log/20260918T113930Z-recipe-astarte-upstream-proposed.md` |
| Task line appended | `.mule/todo.md:173` — `compat-note-v1.3.4` |
