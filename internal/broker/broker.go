...
// DisconnectDevice force-closes a device's live MQTT session, if any
// (device deletion kicks the session before wiping the rows, mirroring
// upstream's disconnect-first deletion order). Unknown or disconnected
// devices are a no-op.
func (b *Broker) DisconnectDevice(realm string, id deviceid.ID) {
	sess := b.registry.get(Identity{Realm: realm, DeviceID: id}.CN())
	if sess == nil {
		return
	}
	_ = b.srv.DisconnectClient(sess.client, packets.ErrNotAuthorized)
}
