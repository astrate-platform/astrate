...
func (c *schemaCache) buildRealm(ctx context.Context, realmID int16, name string) (*realmSchema, error) {
	...
	storedPolicies, err := c.st.ListTriggerPolicies(ctx, realmID)
	if err != nil {
		return nil, fmt.Errorf("engine: loading trigger policies of realm %s: %w", name, err)
	}
	policyByName := make(map[string]*triggers.Policy, len(storedPolicies))
	for i := range storedPolicies {
		p, err := triggers.CompilePolicy(storedPolicies[i].Definition)
		if err != nil {
			c.log.Error("stored trigger policy does not compile",
				"realm", name, "policy", storedPolicies[i].Name, "err", err)
			continue
		}
		// Keyed by the stored row's name, which is what a trigger's "policy"
		// member references; the compiled document's name agrees with it by
		// construction, but the row is the identity.
		policyByName[storedPolicies[i].Name] = p
	}
	...
	for i := range storedTriggers {
		...
		if tr.PolicyName != "" {
			if p, ok := policyByName[tr.PolicyName]; ok {
				tr.AttachPolicy(p)
			} else {
				c.log.Warn("trigger names an unknown delivery policy; using default delivery behaviour",
					"realm", name, "trigger", tr.Name, "policy", tr.PolicyName)
			}
		}
