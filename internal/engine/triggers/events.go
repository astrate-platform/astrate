...
// what upstream's DateTime.from_unix(ms) |> Jason.encode produces. Exported
// because consumers outside the executor (the Channels socket) render the
// same instant into the same upstream shape.
...
// NewDeviceErrorEvent builds a device_error event body from one of Astrate's
// own reject-reason labels (§2.6). The reason is translated to the upstream
// error_name enum by UpstreamErrorName, because consumers validate error_name
// against that closed set and drop any event carrying an unknown name — the
// Dashboard's Device Live Events card raises "Unrecognised event received".
// The untranslated reason is preserved under metadata["astrate_reason"]
// whenever translation changed it, so no diagnostic detail is lost. A nil
// metadata map renders as {} (upstream always emits the map).
func NewDeviceErrorEvent(errorName string, metadata map[string]string) DeviceErrorEvent {
	upstream := UpstreamErrorName(errorName)
	out := make(map[string]string, len(metadata)+1)
	for k, v := range metadata {
		out[k] = v
	}
	if upstream != errorName {
		out["astrate_reason"] = errorName
	}
	return DeviceErrorEvent{Type: OnDeviceError, ErrorName: upstream, Metadata: out}
}
