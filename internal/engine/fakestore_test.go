...
type fakeStore struct {
	...
	// policiesByRealm feeds ListTriggerPolicies.
	policiesByRealm map[int16][]store.TriggerPolicy
...
func (f *fakeStore) ListTriggerPolicies(_ context.Context, realmID int16) ([]store.TriggerPolicy, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	out := make([]store.TriggerPolicy, len(f.policiesByRealm[realmID]))
	copy(out, f.policiesByRealm[realmID])
	return out, nil
}
...
// addPolicy installs a trigger delivery policy into a realm.
func (f *fakeStore) addPolicy(realmID int16, name string, definition []byte) {
	f.mu.Lock()
	f.policiesByRealm[realmID] = append(f.policiesByRealm[realmID],
		store.TriggerPolicy{RealmID: realmID, Name: name, Definition: definition})
	f.mu.Unlock()
}
