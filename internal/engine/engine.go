...
func (e *Engine) fireData(rs *realmSchema, op *PersistOp) {
	...
	e.bus.Publish(stream.Event{
		...
		DeviceID:       deviceID,
		Interface:      op.Interface.Name,
		InterfaceMajor: op.Interface.Major,
		Path:           op.Path,
		Value:          value,
		Timestamp:      op.TS,
	})
...
func (e *Engine) fireDeviceError(m broker.InboundMessage, reason, detail string) {
	// The live bus hears about every rejection, including one in a realm that
	// declares no trigger at all — a Channels viewer watching device_error must
	// not depend on a trigger existing. The trigger path keeps its own guard.
	e.bus.Publish(stream.Event{
		Kind:          stream.KindDeviceError,
		Realm:         m.Realm,
		DeviceID:      m.DeviceID.String(),
		Timestamp:     m.ReceivedAt,
		ErrorName:     reason,
		ErrorMetadata: map[string]string{"detail": detail},
	})
