...
// SeriesSpan reports the timestamps of the oldest and newest samples of an
// individual-datastream series under q's filters. ok is false when the filters
// select no rows, in which case first and last are the zero time.
func (s *Store) SeriesSpan(ctx context.Context, q SeriesQuery) (first, last time.Time, ok bool, err error) {
	...
	if err := s.pool.QueryRow(ctx, fmt.Sprintf(`
		...
		FROM individual_datastreams
		%s`, where), args...).Scan(&f, &l); err != nil {
		return time.Time{}, time.Time{}, false, fmt.Errorf("store: reading series span %s: %w", q.Path, err)
	}
	if f == nil || l == nil {
