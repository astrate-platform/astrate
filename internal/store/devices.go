...
// DeviceStats counts a realm's devices and how many are currently connected
// (the AppEngine /stats/devices endpoint the dashboard polls).
func (s *Store) DeviceStats(ctx context.Context, realmID int16) (total, connected int64, err error) {
	if err := s.pool.QueryRow(ctx,
		`SELECT count(*), count(*) FILTER (WHERE connected) FROM devices WHERE realm_id = $1`,
		realmID).Scan(&total, &connected); err != nil {
		return 0, 0, fmt.Errorf("store: device stats: %w", err)
	}
	return total, connected, nil
}

// DeleteDevice removes the device row and every stored datum in one
// transaction (synchronous delete — Astrate is single-process, so upstream's
// async deletion_in_progress flow is unnecessary; docs/COMPATIBILITY.md).
// Properties and group memberships cascade via FK; the datastream hypertables
// are swept explicitly, mirroring DeleteRealm. An unknown device yields
// ErrNotFound.
func (s *Store) DeleteDevice(ctx context.Context, realmID int16, id deviceid.ID) error {
	tx, err := s.pool.Begin(ctx)
	if err != nil {
		return fmt.Errorf("store: beginning device delete: %w", err)
	}
	defer func() { _ = tx.Rollback(ctx) }()

	for _, table := range []string{"individual_datastreams", "object_datastreams"} {
		if _, err := tx.Exec(ctx,
			`DELETE FROM `+table+` WHERE realm_id = $1 AND device_id = $2`,
			realmID, uuidParam(id)); err != nil {
			return fmt.Errorf("store: deleting %s of device %s: %w", table, id, err)
		}
	}
	tag, err := tx.Exec(ctx, `DELETE FROM devices WHERE realm_id = $1 AND id = $2`,
		realmID, uuidParam(id))
	if err != nil {
		return fmt.Errorf("store: deleting device %s: %w", id, err)
	}
	if tag.RowsAffected() == 0 {
		return fmt.Errorf("%w: device %s", ErrNotFound, id)
	}
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("store: committing device delete: %w", err)
	}
	return nil
}
