...
// ListDeviceGroupsBatch resolves group names for a page of devices in one
// query (the details=true device listing would otherwise go N+1).
func (s *Store) ListDeviceGroupsBatch(ctx context.Context, realmID int16, ids []deviceid.ID) (map[deviceid.ID][]string, error) {
	if len(ids) == 0 {
		return map[deviceid.ID][]string{}, nil
	}
	params := make([]pgtype.UUID, len(ids))
	for i, id := range ids {
		params[i] = uuidParam(id)
	}
	rows, err := s.pool.Query(ctx, `
		SELECT gd.device_id, g.name FROM groups g
		JOIN group_devices gd ON gd.group_id = g.id
		WHERE gd.realm_id = $1 AND gd.device_id = ANY($2)
		ORDER BY g.name`,
		realmID, params)
	if err != nil {
		return nil, fmt.Errorf("store: batch-listing device groups: %w", err)
	}
	defer rows.Close()

	out := make(map[deviceid.ID][]string, len(ids))
	for rows.Next() {
		var (
			u    pgtype.UUID
			name string
		...
		if err := rows.Scan(&u, &name); err != nil {
			return nil, fmt.Errorf("store: scanning device group row: %w", err)
		}
		id := deviceid.ID(u.Bytes)
