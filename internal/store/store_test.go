...
func TestStore(t *testing.T) {
	...
	t.Run("CompressionLive", func(t *testing.T) { testCompressionLive(t, s) })
	...
	t.Run("UserBlocks", func(t *testing.T) { testUserBlocks(t, s) })
	t.Run("Flows", func(t *testing.T) { testFlows(t, s) })
...
func testMigrations(t *testing.T, s *Store) {
	...
	if version != 12 || dirty {
		t.Fatalf("schema_migrations: got version=%d dirty=%v, want version=12 dirty=false", version, dirty)
	}
	...
	var policiesTable bool
	if err := s.pool.QueryRow(ctx,
		`SELECT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'trigger_policies')`).
		Scan(&policiesTable); err != nil {
		t.Fatalf("querying trigger_policies table: %v", err)
	}
	if !policiesTable {
		t.Error("trigger_policies table missing (migration 000006)")
	}
}
...
func testMigrationCycle(t *testing.T, s *Store) {
	...
	if version != 12 || dirty {
		t.Fatalf("after cycle: got version=%d dirty=%v, want version=12 dirty=false", version, dirty)
	}
}
