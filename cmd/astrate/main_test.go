...
func bootConfig(t *testing.T, dsn, httpAddr, sessPath, realmName, pub string) config.Config {
	...
	cfg.HTTP.CORSAllowedOrigins = []string{"http://dashboard.example:4040"}
	cfg.Log.Level = "error"
	return cfg
}
...
func TestBoot(t *testing.T) {
	...
	t.Run("HealthAndMetrics", func(t *testing.T) {
		...
		// Upstream-parity per-service health, polled by the dashboard.
		for _, svc := range []string{"appengine", "realmmanagement", "pairing"} {
			if code, _ := get(t, base+"/"+svc+"/health"); code != http.StatusOK {
				t.Errorf("%s/health = %d, want 200", svc, code)
			}
		}
	...
	t.Run("CORSPreflight", func(t *testing.T) {
		req, _ := http.NewRequest(http.MethodOptions, base+"/appengine/v1/"+realmName+"/devices", nil)
		req.Header.Set("Origin", "http://dashboard.example:4040")
		...
		if got := resp.Header.Get("Access-Control-Allow-Origin"); got != "http://dashboard.example:4040" {
