...
Default `--skip-intro` mashes A/START plus direction probes until the player
actually moves (Oak preloads bedroom coords before free play — non-zero WRAM
alone is not enough). Use `--no-skip-intro` on the agent CLI to disable.
ControlCommands are queued
