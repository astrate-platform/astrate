...
defmodule AstarteDevTool.Data.Realm do
  require Logger
  ...
  def create(realm_name, public_key_pem, replication, device_limit, max_retention, opts) do
  ...
  def delete(realm_name, opts \\ []) do
    keyspace_name =
      CQLUtils.realm_name_to_keyspace_name(realm_name, Config.astarte_instance_id!())

    if opts[:async] do
      {:ok, _pid} = Task.start(fn -> do_delete(realm_name, keyspace_name) end)

      :ok
    else
      do_delete(realm_name, keyspace_name)
    end
  end
  ...
  defp do_delete(realm_name, keyspace_name) do
  ...
  defp execute_realm_deletion(conn, realm_name, keyspace_name) do
    with :ok <- delete_keyspace(conn, keyspace_name),
  ...
  defp do_create(
    ...
    Xandra.Cluster.run(:xandra, [timeout: 60_000], fn conn ->
      with :ok <- validate_realm_name(realm_name),
           :ok <- create_keyspace(conn, keyspace_name, replication_map_str),
           {:ok, keyspace_conn} <- build_keyspace_conn(conn, keyspace_name),
           :ok <- create_kv_store(keyspace_conn),
  ...
  defp delete_keyspace(conn, realm_name) do
    query = """
  ...
  defp create_keyspace(conn, realm_name, replication_map_str) do
    query = """
  ...
  defp create_kv_store({conn, realm}) do
    query = """
