...
func cmdProvision(args []string) error {
	...
	// Upstream publishes the realm's key asynchronously; see awaitRealmKey.
	fmt.Print("waiting for the realm signing key to become usable…")
	if err := c.awaitRealmKey(ep, realmKey, *realm, 2*time.Minute); err != nil {
		fmt.Println()
		return err
	}
	fmt.Println(" ok")
