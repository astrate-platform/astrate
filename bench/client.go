...
// createRealm creates the realm, or — when it already exists — re-keys it to
// the realm key this run generated, so provision stays re-runnable.
//
// Upstream Astarte 1.2.0 answers an existing realm with **422 and
// `existing_realm`**, not 409, and it does not accept PUT on a realm; PATCH is
// the update verb. Both were measured against v1.2.0 (2026-07-26). Re-keying
// rather than bailing out matters because provision generates a fresh realm
// signing key on every run: leaving the old public key in place would make
// every subsequent realm-management and pairing call fail with 403.
func (c *client) createRealm(ep Endpoints, hkKey crypto.Signer, realm, realmPubPEM string) error {
	...
	patchErr := c.doJSON(http.MethodPatch, ep.Housekeeping+"/v1/realms/"+realm, token, map[string]any{
		"jwt_public_key_pem": realmPubPEM,
	}, nil)
	if patchErr == nil {
		return nil
	}
	// Astrate's housekeeping has no realm-update route at all, so there is no
	// way to hand it the key this run generated. Say so here rather than
	// letting it surface later as an unexplained 403 on the first realm call.
	if isStatus(patchErr, http.StatusNotFound) || isStatus(patchErr, http.StatusMethodNotAllowed) {
		return fmt.Errorf("realm %q already exists and this deployment cannot be re-keyed "+
			"(no PATCH on /housekeeping/v1/realms/%s); provision generates a fresh signing key "+
			"each run, so delete the realm or provision under a different -realm name", realm, realm)
	}
	return patchErr
}

// awaitRealmKey blocks until the realm's signing key is usable for
// realm-management calls.
//
// Realm creation upstream is **asynchronous**: housekeeping answers 201 before
// the realm's public key is readable by the other services, and a
// realm-management call made in that window fails with a bare 403 that is
// indistinguishable from an authorization error. Measured against v1.2.0: 403
// at 0.4s after the 201, 200 at 2.5s. Without this wait, provision against a
// freshly created realm always fails on the first interface install.
func (c *client) awaitRealmKey(ep Endpoints, realmKey crypto.Signer, realm string, timeout time.Duration) error {
	...
	for {
		err = c.doJSON(http.MethodGet, ep.RealmMgmt+"/v1/"+realm+"/interfaces", token, nil, nil)
		if err == nil {
			return nil
		}
		if !isStatus(err, http.StatusForbidden) || time.Now().After(deadline) {
			return fmt.Errorf("waiting for realm %q to become usable: %w", realm, err)
		}
