...
For your own deployment: run `astarte/astarte-dashboard:v1.2.2` with a
`config.json` pointing `astarte_api_url` at Astrate's HTTP address, add the
dashboard's origin to `http.cors_allowed_origins`
(`ASTRATE_HTTP_CORS_ALLOWED_ORIGINS`), and mint tokens against your own realm
key. The Device Live Events card requires the Channels socket (planned M11);
